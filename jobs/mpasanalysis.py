...
class MPASAnalysis(Diag):
    ...
    def execute(self, config, event_list, dryrun=False):
        ...
        generate_string = '["' + '", "'.join([x for x in mpas_config.get('generate_plots', '')]) + '"]'
        
        # setup the start year offset for nino and ts plots
        if self._start_year == int(config['simulations']['start_year']):
            start_offset = 0
        else:
